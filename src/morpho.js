 ...
 */
export async function listMarkets(network, { first = 20 } = {}) {
  const config = NETWORKS[network?.toLowerCase()]
  if (!config) {
    throw new MorphoError(
      'UNSUPPORTED_NETWORK',
  ...
  return {
    ...
    vaults: items.map((v) => ({
      ...
      totalAssetsUsd: v.state?.totalAssetsUsd
        ? `$${Number(v.state.totalAssetsUsd).toFixed(0)}`
        : '$0',
 ...
 */
export async function wrapEth({ amount, network, rpcUrl }) {
 ...
 */
export async function approve(token, { amount, spender, network, rpcUrl }) {
  if (!token) throw new MorphoError('MISSING_TOKEN', 'token address is required')
